...
\subsection{Outcome}
	...
	As established in the previous subsection {\tt Outcomes} represent the possible values that a variable (i.e. an {\tt Event}) can take on. Each {\tt Outcome} is thus characterised by its basic {\it name}, {\it description}, and the {\it event instance} of which it is part. 
...
\item {\tt DataSet.reveal\_interval}
	...
	Specifies the total duration in days of each individual challenge, or alternatively the time between consecutive revelations. 

\end{itemize}

Using the various methods that use and wrap around those fields it is easy for [TODO]
...
\end{figure}
	...
	As visible in Figure \ref{fig:admin-index}, the admin interface is structured in sections corresponding to the individual {\it models} accessible from it. Inside the code this is achieved by introducing special classes that derive from the base {\tt ModelAdmin} prototype and are linked to each of the said models. Each class in turn specifies the fields and operations that can be accessed, modified or listed by administrators, and allows the handling of particular events of interest related to the admin interface. As a Django convention all such objects must be declared in the {\tt admin.py} file within the {\tt markets} project. 

	The main functionality that the admin interface provides is the creation, modification and deletion (i.e. CRUD operations) of {\it markets}, {\it events} and {\it data-sets}. As can be seen in the market class diagram (Figure \ref{fig:class-diagram}) though, the last two of these models are in turn comprised of other objects, i.e. events with their possible {\it outcomes} and data-sets with the underlying {\it data} and the {\it results} of each datum. 

\subsection{Markets}

	As discussed in Section \ref{sub:market}, even though the {\it market} model is the central and arguably the most important model in the software, it is quite uncomplicated in terms of its fields and the methods it implements. As a consequence, the respective {\tt MarketAdmin} class needs only support simple CRUD operations manipulating the few fields it possesses; functionality that can be implemented in a few lines using the Django framework. Figure ??? shows the admin form displayed when adding a new market.  
