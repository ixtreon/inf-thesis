...
In the light of the recently emerging concepts of information economy and big data, out understanding of the value of information and knowledge has changed dramatically. New means of quantifying the value of information are needed and this problem has been particulatly well-established in the area of machine learning. Solutions to the paradigm of faithfully estimating the "price of information" has been provided by the advances in the area of machine learning prediction markets. These markets prove superior to previous solution to the same problem by incencitising and encouraging players to share their " true" knowledge with the market. For detailed discussion, please refer to Chapter 1. 

My project consists of the implementation of flexible online prediction and machine learning market framework. Its key feature is that it is constructed as an extensible base which can be used in the construction of prediction market and model aggregation, as well as in the organisation of collaborative crowdsourced machine learning market contests (discussed in Chapter 6). 

I started constructng the market from scratch (motivation detailed in Chapter 3) and during the limited time I had to work on the project I managed to create a simple yet expressive prediction market framework, allowing placing of orders (bets) and market management and developed a modular market maker architecture. Thus the software I developed is flexible in terms of its ability not only to implement and configurate number of different market makers, but also to allow writing new ones. I chose Python as my main design environment due to reasons outlined in Chapter 5. One of the most important goals that I had in mind while working on the project was to implement fuctional and friendly user interface (UI). I used Django for designing the web framework. In its finalised form, the UI provides all functionalities expected from an online trading platform - market listings, registration, forms for placing and cancelling orders. I have always intended to deliver the software framework in a form that can be used as a research tool, allowing the execution of experiments where one or more participants are automated. This goal was achieved by the use of API to manage and interact with prediction markets.

Although the software framework is complete in its functionality, I have identified directions for future work and improvements. These include efforts to implement dynamic market UI and to allow composition of joined events. Such improvements will make the software better suited for the applications descibed in Chapter 6. The good news is that my project framework is designed to be easily extendable. 
...
\flushleft {Each row contains the name of a software component used by the market framework, followed by the versions used while writing and testing it. }
\end{table}
...
% better than other guys since has many events w/ many outcomes and many datasets

\subsection{Functionality}
% supports custom market makers; existing solutions support 1 or 2 fixed makers, I take a modular approach

% while its main use case is modelling prediction markets, the framework allows the creation of more general types of markets. 

\subsection{Interface}

% user interface as full-featured, yet uncomplicated

% stress the importance of the RESTful API in terms of the possible use cases
...
\chapter{Conclusion}
Hereby, I descibed the successful design and implementation of a flexible online prediction and machine learning framework. 

The need for such frameworks is defined in the light of the recent intruduction of concepts such as information economy and big data which emphasise on the need to find novel, more acurate means of quantifying the value of information. In the field of machine learning, several approaches to tackle this problem have been identified. For example, meta-algorithms, also called {\em ensemble methods} are being used in model aggregation to elicit more reliable prediction of any of its constituent algorithms. 

The issue of reliably estimating the "cost" of information has been addressed by extensive research and recent advances in the area of machine learning prediction markets. The concept of machine learning prediction market ahs been pioneereed in 2010 and, as the name suggests draw parallels between prediction markets amd machine learning approaches for model combination. Although the predictive power of such markets has been established only theoretically, the fact that prediction markets alone have been shown to outperform public opinion polls in predictiong the results from presidential election or the recent Scottish referendum. 

Having decided not to use any of the existing open-source platforms as a reference point for my project, I constucted a strict plan to follow throughout the duration of the project. Although some of the points addressed in the plan were a subject to change, I nevertheless managed to include all functionalities that I consider essential in terms of project goals. I designed from scratch an expressive prediction market framework which allows placing of orders (bets) using a human-facing web interface and market management using Python commands or admin interface. These features allow for the modulation of current challenges, events and market maker modules. 

Market makers are vital to the predictive performance of their corresponding markets. Thus a requirement for a well-performing prediction market framework is to either implement a number of existing, well-studied and possibly configurable market maker schemes or to allow writing new ones. My project satisfies both these requirements by [explain how]. I have implemented a number of sample market ({\em order book} and {\em logarithmic market scoring rule} makers demostrating the capabilities of the created software framework and its extendability. Although I realise the importance of user interface as far as player engagement is conserned, I put more work into constructing the software framework and made the decision that a simple and static yet functional user interface will be sufficient for the assigned goals for the project. I have implemented the functionalities required from an online trading platform. UI exptensions are outlined in Chapter 4. An important feature that I implemented into my software framework was to allow programmatic access via using API as a linked to manage and interact with the prediction markets. Thus feature brings about the extensibility of the accomplished software and outlines how it can be useful for the creation of model aggregation approaches or for the organisation of collabortaive crowdsourced machine learning contests. [include a sentence or two about possible applications of these approaches and how they can be used to advance knowledge].

A drowback of the software framework in its current state is that it only supports disjoint events thus its implemnetation for the organisation of collaborative machine learning contests can be obscured. Having realised this issue, I have put on my schedule to allow event composition in the market framework. Allowing creation of event pairs or triplicates is necessary step towards achieving a flexible prediction market framework.  

The described software is a relatively straightforward yet comprehensive example of flexible online prediction and ML market framework. Its key design feature is to serve as an extensible base which can be imporoved to accomodate and solve more complicated challenges from the field of machine learning prediction markets.
